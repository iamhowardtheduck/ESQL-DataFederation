...
# ---------------------------------------------------------------------------
# Shared identity layer: customers draw first/last names from the same pools
# as the HR roster (a separate hash namespace — shoppers and employees are
# different populations that merely share name vocabularies), so a repeat
# CUST-n keeps ONE name across every order instead of re-rolling per document.
# ---------------------------------------------------------------------------
...
def _load_garage_s3():
    ...
    if not os.path.exists(path):
        return None
    spec = _ilu.spec_from_file_location("garage_s3", path)
    mod = _ilu.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def customer_name(cust_n):
    """Stable (first, last) for a customer id. Falls back to the local pools
    if identity.py is absent so the script still runs standalone."""
    if IDENT is None:
        import random as _r
        rng = _r.Random(cust_n)
        return rng.choice(FIRST), rng.choice(LAST)
    n = IDENT._h("customer", cust_n)
    return (IDENT.FIRST_NAMES[n % len(IDENT.FIRST_NAMES)],
            IDENT.LAST_NAMES[(n >> 16) % len(IDENT.LAST_NAMES)])
...
def build_order(seq, now, start, end, cust_pool):
    ...
    first, last = customer_name(cust_n)
    tier = wpick(TIERS)
...
def main():
    ...
    if S3 is not None:
        S3.add_args(ap, default_prefix="orders")
    else:
